...
export default class Registrar {
  constructor({
    ...
    console.log('create registrar - ens, registrar, controller', registryAddress, permanentRegistrar, permanentRegistrarController);
  ...
  async checkCommitment(label, secret = '') {
    ...
    console.log('LIB:CHECK_COMMIT', label, account, secret, commitment);
  ...
  async commit(label, secret = '') {
    ...
    const account = await getAccount()
    const commitment = await this.makeCommitment(label, account, secret)
    console.log('LIB:COMMIT', commitment);
  ...
  async register(label, duration, secret) {
    ...
    console.log('LIB:REGISTER',
      ...
      resolverAddr,
      account,
      { value: priceWithBuffer, int: parseInt(priceWithBuffer, 16) }
    );
    if (parseInt(resolverAddr, 16) === 0) {
...
export async function setupRegistrar(registryAddress) {
  ...
  // let tldRegistrarAddress = process.env.REACT_APP_REGISTRAR
  let tldRegistrarAddress = await ENS.owner(namehash(process.env.REACT_APP_REGISTRAR_TLD))
  ...
  let controllerAddress = process.env.REACT_APP_REGISTRAR_CONTROLLER;
